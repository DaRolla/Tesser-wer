...
public class Constants {
     ...
     *-------------------------------------------------------------------------*/
    /*
     * For Pix, Box, Pta and Numa, there are 3 standard methods for handling the retrieval or insertion of a struct: (1)
     * direct insertion (Don't do this if there is another handle somewhere to this same struct!) (2) copy (Always safe,
     * sets up a refcount of 1 on the new object. Can be undesirable if very large, such as an image or an array of
     * images.) (3) clone (Makes another handle to the same struct, and bumps the refcount up by 1. Safe to do unless
     * you're changing data through one of the handles but don't want those changes to be seen by the other handle.) For
     * Pixa and Boxa, which are structs that hold an array of clonable structs, there is an additional method: (4)
     * copy-clone (Makes a new higher-level struct with a refcount of 1, but clones all the structs in the array.)
     * Unlike the other structs, when retrieving a string from an Sarray, you are allowed to get a handle without a copy
     * or clone (i.e., that you don't own!). You must not free or insert such a string! Specifically, for an Sarray, the
     * copyflag for retrieval is either: TRUE (or 1 or L_COPY) or FALSE (or 0 or L_NOCOPY) For insertion, the copyflag
     * is either: TRUE (or 1 or L_COPY) or FALSE (or 0 or L_INSERT) Note that L_COPY is always 1, and L_INSERT and
     * L_NOCOPY are always 0.
     */
    ...
    /*
     * The IFF_DEFAULT flag is used to write the file out in the same (input) file format that the pix was read from. If
     * the pix was not read from file, the input format field will be IFF_UNKNOWN and the output file format will be
     * chosen to be compressed and lossless; namely, IFF_TIFF_G4 for d = 1 and IFF_PNG for everything else. IFF_JP2 is
     * for jpeg2000.
     */
